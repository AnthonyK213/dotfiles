...
function M.get_pos(sentence, position)
  ...
  if _nviq_jieba.nviq_jieba_pos(M.jieba, sentence, position, s, e) == 0 then
    return s[0], e[0] - 1
  end
end
...
function M.get_word()
  ...
  if not s_char or not e_char then
    lib.warn("Failed to tokenize the line")
    return "", pos_byte, pos_byte
  end
...
function M:enable()
  ...
  if not self.jieba then
    ...
    if not self.jieba then return false end
